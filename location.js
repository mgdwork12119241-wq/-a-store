(() => {
  ...
  function initMap() {
    ...
    map.on('load',loadStores); map.on('click',(e)=>selectLocation(e.lngLat.lat,e.lngLat.lng)); requestLocation();
  }
  ...
  function selectLocation(lat,lng,fromGps=false) {
    ...
    if(userMarker)userMarker.remove(); userMarker=new maplibregl.Marker({color:'#111827'}).setLngLat([lng,lat]).addTo(map);
  ...
  function confirmLocation() {
    if(!selected)return; localStorage.setItem('delivery_location',JSON.stringify(selected));
    const pill=document.querySelector('.location-pill span'); if(pill)pill.textContent='موقع التوصيل محدد ✓';
    if(typeof closeModal==='function')closeModal(); toastSafe('تم حفظ موقع التوصيل ✅');
  }
  async function loadStores() {
    ...
    if(error||!data)return; storeMarkers.forEach(m=>m.remove());
  ...
  function setupAppDrawer(){
    ...
    const style=document.createElement('style'); style.textContent=`
      ...
      .options-menu.storey-open{transform:translateX(0);opacity:1;visibility:visible}.options-menu button{display:block!important;width:100%!important;text-align:right!important;border:0!important;background:transparent!important;border-radius:14px!important;padding:13px 14px!important;margin:2px 0!important;font:600 14px inherit!important;color:#202532!important}.options-menu button:hover,.options-menu button:active{background:#f3f5f8!important}.options-menu .drawer-title{font-size:12px;font-weight:800;color:#8a91a0;padding:16px 14px 7px;border-top:1px solid #eef0f4;margin-top:8px}.storey-scrim{position:fixed;inset:0;background:rgba(10,14,23,.38);backdrop-filter:blur(2px);z-index:90;opacity:0;visibility:hidden;transition:.25s}.storey-scrim.storey-open{opacity:1;visibility:visible}body.storey-drawer-open{overflow:hidden}.browse-page{width:92%;max-width:1100px;margin:35px auto 80px}.browse-head{margin-bottom:24px}.browse-head h1{font-size:34px;margin:4px 0}.browse-back{border:0;background:#eef0f4;border-radius:12px;padding:9px 13px;font:700 12px inherit;cursor:pointer;margin-bottom:18px}.empty-browse{grid-column:1/-1;background:#fff;border:1px solid #e8eaf0;border-radius:20px;padding:35px;text-align:center}.empty-browse p{color:#777e8c;font-size:12px}@media(max-width:600px){.options-menu{width:88vw!important}.options-menu button{padding:14px!important;font-size:14px!important}.browse-page{margin-top:22px}.browse-head h1{font-size:28px}}
    `; document.head.appendChild(style);
    const menu=$('#optionsMenu'),btn=$('#menuBtn'); if(!menu||!btn)return; menu.id='storeyDrawer';
    const title=document.createElement('div'); title.className='drawer-title'; title.textContent='تصفح'; menu.appendChild(title);
    ...
    menu.querySelectorAll('.drawer-category').forEach(b=>b.addEventListener('click',()=>{close();openCategoryPage(b.dataset.drawerCategory)}));
    menu.querySelectorAll('button:not(.drawer-category)').forEach(b=>b.addEventListener('click',close));
  }

  function openCategoryPage(category){
    let page=$('#storeyBrowsePage');
    if(!page){page=document.createElement('section');page.id='storeyBrowsePage';page.className='browse-page';const main=document.querySelector('main');main.appendChild(page)}
    $('#hero')?.setAttribute('hidden','');$('#categoriesSection')?.setAttribute('hidden','');$('#featuredSection')?.setAttribute('hidden','');$('#joinBanner')?.setAttribute('hidden','');$('#resultsSection')?.setAttribute('hidden','');
    page.hidden=false;
    const title=category==='all'?'كل الفئات':category;
    page.innerHTML=`<button class="browse-back" id="browseBack">→ العودة للمساعد</button><div class="browse-head"><span class="mini-label">تصفح</span><h1>${escapeHtml(title)}</h1><p class="muted">اختر متجراً لعرض المنتجات المتاحة.</p></div><div class="store-grid" id="browseGrid"></div>`;
    const list=category==='all'?stores:stores.filter(s=>s.cat===category); const grid=$('#browseGrid');
    grid.innerHTML=list.length?list.map(card).join(''):`<div class="empty-browse"><strong>لا توجد متاجر ضمن هذه الفئة حالياً.</strong><p>يمكنك العودة للمساعد وتجربة بحث آخر.</p></div>`;
    if(typeof bindCards==='function')bindCards(grid);
    $('#browseBack').onclick=()=>{page.hidden=true;$('#hero')?.removeAttribute('hidden');$('#categoriesSection')?.removeAttribute('hidden');$('#featuredSection')?.removeAttribute('hidden');$('#joinBanner')?.removeAttribute('hidden');window.scrollTo({top:0,behavior:'smooth'})};
  ...
  document.addEventListener('DOMContentLoaded',setupAppDrawer); setTimeout(setupAppDrawer,50);
})();